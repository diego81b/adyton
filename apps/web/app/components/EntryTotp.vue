...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-1.5">
      <span class="text-[11px] font-mono uppercase tracking-wider text-dimmed">TOTP</span>
      <span class="text-[11px] font-mono text-dimmed">rotates every 30s</span>
    </div>

    <div v-if="error" class="text-sm text-error">Invalid TOTP secret (not valid base32).</div>

    <div v-else class="flex items-center gap-3">
      <div class="relative w-10 h-10 shrink-0">
        <div class="totp-ring absolute inset-0 rounded-full" :style="ringStyle" />
        <div
          class="absolute inset-0 flex items-center justify-center text-[11px] font-mono font-semibold text-muted"
        >{{ remaining }}</div>
      </div>
      <span class="flex-1 text-3xl font-mono font-semibold tracking-[0.2em] text-primary">{{ display }}</span>
      <UButton
        color="neutral"
        variant="ghost"
        size="sm"
