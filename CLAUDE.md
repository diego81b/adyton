...
**Phase 5 COMPLETE + verified (2026-06-04).** Int tests run (settings/account-deletion/cors pass; only failures = 2 pre-existing 429 rate-limit ones), snapshots regenerated, full browser smoke of `/generator` + `/settings` done. **CORS fix landed:** `@fastify/cors` v10 defaults to GET,HEAD,POST — explicit `methods` list in `create-app.ts` now allows PUT/PATCH/DELETE (regression test `cors.int-spec.ts`); without it every browser mutation except POST failed preflight. Next: Phase 6 (2FA).
