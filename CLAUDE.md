...
**Enterprise restyle v2 — Swiss/minimal (2026-06-13, browser-verified both themes, commit `4099fd1`):** superseded a rejected first pass ("posticcia"). Design-first (ran `ui-ux-pro-max` → Swiss/Minimal enterprise + `web-design-guidelines`); user chose **single-scroll dense** settings + **Swiss minimal** density. **ONE layout language across settings + vault list + detail** (see `design-system.md` §8): single radius scale (`rounded-lg` containers / `rounded-md` chips — NO `rounded-xl`/`2xl` on app screens), 3-level elevation (page / one hairline `divide-y` container / rows — no card-in-card), quiet uppercase section labels, accent **only** on the single primary CTA + active segment + `focus-visible` + status dot, `tabular-nums` on data. New primitives **`SettingsGroup.vue`** (uppercase header + hairline divided container) + **`SettingRow.vue`** (`flex-wrap` label/helper/value/`#action` — wrap is load-bearing: a long value can never squeeze the action out, the bug that hid Revoke). Settings rewritten to a **single column `max-w-3xl`** (the old 2-col grid put `SessionsCard` in a half-width column → Revoke overflow): 2FA/Biometric → rows in a Security group; Passkeys/Sessions/TrustedDevices → each its own `SettingsGroup`; AutoLock → 2 rows; Account/Export/Import → rows; danger zone separated. **`SettingsSection.vue` deleted.** Detail page aligned to the list (neutral tile `size-12 rounded-lg`, hairline divided `DetailField`s, action bar = solid **Edit** / ghost **History** / error-ghost **Delete** `ml-auto` — was 4 equal `flex-1`). OTP: new **`OtpInput.vue`** (6 segmented boxes, one transparent overlaid `<input>` for paste + OS one-time-code autofill) wired into `TwoFactorChallenge` + `TwoFactorSetupModal`. **Icon gotcha (again): `i-lucide-log-out`/`shield-x` are NOT in the @nuxt/icon client bundle → CSP-blocked remote fetch (icon invisible); `.nuxt` clear + restart did NOT add them — use a bundled icon (swapped to `i-lucide-shield-off`).** 445 web tests green (specs migrated from badge-text → status-dot/aria-label assertions). Dev throwaway account `uitest-restyle@adyton.local`.
